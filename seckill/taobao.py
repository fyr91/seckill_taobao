...
class taobao():
    ...
    def iselement(browser, xpaths):
        ...
        """
        try:
            browser.find_element("xpath", xpaths)
            return True
        except:
            return False
    ...
    def openChromeToTaobao(self):
        ...
        driver.find_element("xpath", '//i[@class="iconfont icon-qrcode"]').click()
        # 给我5s扫码登录的时间
        # Getting HTML
        print('这里扫码开始等待点击')
        time.sleep(5)
        while 1==1:
            ...
            if isLogin:
                ...
                # 这里的参数需要修改,勾选的商品!
                i = 2
                driver.find_element("xpath", f'/html/body/div[1]/div[3]/div/div/div[2]/div[2]/div[{i}]/div/div[1]/div/div').click()
                print('正在等待秒杀时间')
                while 1==1:
                    # 本地时间
                    ...
                    if tTime >times:
                        # 点击结算按钮
                        driver.find_element("xpath", '/html/body/div[1]/div[3]/div/div/div[4]/div[2]/div[3]/div[5]').click()
                        while 1==1:
                            ss=taobao.iselement(driver,'//*[@id="submitOrderPC_1"]/div/a[2]')
                            if ss:
                                #点击提交按钮
                                driver.find_element("xpath", '//*[@id="submitOrderPC_1"]/div/a[2]').click()
                                print('vsign抢到时间: ',datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f'))
                                return None
                    else:
                        print(f"requesting taobao time: {tTime}")
                        # time.sleep(0.1)
